...
import { CONTRACT_MAP_MULTICALL } from '../constants'
...
class MulticallContract extends Client {
  ...
  async getEVMNativeBalances(addressList: Array<Address>) {
    const multicallContract = CONTRACT_MAP_MULTICALL[this.chain.id] || MULTICALL_CONTRACT

    return await this.getEVMNativeBalancesFromMulticall(addressList, multicallContract)
  }

  async getEVMNativeBalancesFromMulticall(addresss: Array<Address>, multicallAddress: Address) {
  ...
  async getErc20BalancesFromMulticall(tokenAddress: Address, addresss: Array<string>) {
