...
import { TRANSFER_MAP } from "src/constants";
...
const verifyApprove = async (
  ...
  const allowance = await erc20.getAllowce(address, TRANSFER_MAP[chainId]);
  if (allowance < total) {
    if (autoApprove) {
      logger.loading("Approve");
      const hash = await erc20.approve(TRANSFER_MAP[chainId], total);
...
const sendErc20Token = async (
  ...
  const total = _addressAndAmounts.reduce(
    (sum, { amount }) => sum + amount,
    BigInt(0)
  );
  const isApproved = await verifyApprove(
    !!autoApprove,
    total,
    chainId,
    tokenAddress,
  ...
  const hash = await batchSendToken.batchTransferErc20Token(
    tokenAddress,
    _addressAndAmounts
  );
